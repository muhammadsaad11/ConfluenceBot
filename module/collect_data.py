    ...
    for page in pages:
        ...
        if page[4] > max_tokens_per_page:
            ...
            for content_part in content_list:
                i += 1
                # tokens = get_num_tokens_from_string(content_part, tokenizer_encoding_name)
                # if tokens > 1:
...
def write_csv(confluence_spaces: list, file_name: str) -> pd.DataFrame:
    ...
    for space in confluence_spaces:
        ...
        percent_complete = 0
        progress_text = 'Loading information from the Confluence Space(s) ' + space + ' ...'
        ...
        time.sleep(0.01)
        percent_complete += 5
        progress_text = str(len(confluence_pages)) + ' Page(s) loaded.'
        ...
        time.sleep(0.01)
        percent_complete += 5
        progress_text = 'divided in ' + str(len(confluence_pages)) + ' page(s).'
        progress_bar.progress(percent_complete, text = progress_text)
        ...
        for row in pages_df.itertuples():
            ...
            progress_text = 'Creating embeddings (' + str(i) + ' out of ' + str(total) + ')'
            progress_bar.progress(percent_complete, text = progress_text)

        # Write csv
        file_name_space = get_file_name_for_space(file_name, space)
        print("reached write csv")
        pages_df.to_csv(file_name_space, index = False)

        # Progress indicator
        percent_complete = 100
        progress_text = 'CSV file ' + file_name_space + ' written'
        progress_bar.progress(percent_complete, text = progress_text)
